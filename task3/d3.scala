/*
chcp 65001 && spark-shell -i \spark\d3.scala --conf "spark.driver.extraJavaOptions=-Dfile.encoding=utf-8"
*/
...
var df1 = spark.read.format("com.crealytics.spark.excel")
		...
		"""
		spark.read.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=root")
...
var df2 = spark.read.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=root")
        .option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "s3_2")
		.load()
		...
		"""
var df3 = spark.read.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=root")
